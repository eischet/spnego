...
public class SpnegoAuthenticator extends AuthenticatorBase {
     ...
     */
    public SpnegoAuthenticator(final SpnegoFilterConfig config) throws LoginException, GSSException, PrivilegedActionException {
        ...
        if (config.useKeyTab()) {
            ...
            final CallbackHandler handler = SpnegoProvider.getUsernamePasswordHandler(config.getPreauthUsername(), config.getPreauthPassword());
     ...
     * @throws PrivilegedActionException
     */
    public SpnegoAuthenticator(final String loginModuleName, final SpnegoFilterConfig config) throws LoginException, GSSException, PrivilegedActionException {
        ...
        final boolean hasUsername = null != username && !username.trim().isBlank();
     ...
     * @throws IOException
     */
    public SpnegoPrincipal authenticate(final HttpServletRequest req, final SpnegoHttpServletResponse resp) throws GSSException, IOException {
        ...
        final SpnegoAuthScheme scheme = SpnegoProvider.negotiate(req, resp, basicSupported, this.promptIfNtlm, serverRealm, errorPage);
     ...
     */
    private SpnegoPrincipal doBasicAuth(final SpnegoAuthScheme scheme, final SpnegoHttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            resp.addHeader(Constants.AUTHN_HEADER, Constants.BASIC_HEADER + " realm=\"" + this.serverPrincipal.getRealm() + '\"');
    ...
    private SpnegoPrincipal doLocalhost() {
        ...
        if (null == username || username.isBlank()) {
            ...
            return new SpnegoPrincipal(username + '@' + this.serverPrincipal.getRealm(), KerberosPrincipal.KRB_NT_PRINCIPAL);
        }
    }
